...
switch ( $context ) {
	...
	case ( 'lesson' ):
		// Get the current post (lesson)
	    $lesson = get_post();

	    // Get the associated course ID for this lesson
		...
		<div class="ld-section-heading">
			<h2><?php echo esc_html($lesson_title); ?></h2>
		</div>
	...
	case ( 'topic' ):
		// Get the current post (topic)
	    $topic = get_post();

	    // Get the associated course ID for this topic
	    $topic_id = learndash_get_course_id( $topic->ID );

	    // Get the course title
	    $course_title = get_the_title( $topic_id );
	...
	case 'quiz':
		if ( get_post_type( ( ! empty( $post ) ? $post : '' ) ) === learndash_get_post_type_slug( 'quiz' ) ) {
			// Get the quiz title (same as the current post title, since it's a quiz post type)
			$quiz_title = get_the_title();
			
		?>
		<div class="ld-section-heading">
			<h2><?php echo esc_html($quiz_title); ?></h2>
		</div>
